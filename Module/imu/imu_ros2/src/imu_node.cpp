...
#include <imu_ros2/msg/imu_data.hpp>
...
public:
    IMUNode(const std::string &device, int baud)
        : Node("imu_node")
    {
        pub_ = this->create_publisher<imu_ros2::msg::IMUData>("/imu", 10);
...
private:
    IMUReader reader_;
    rclcpp::Publisher<imu_ros2::msg::IMUData>::SharedPtr pub_;
