...
Rs02MotorDriver::Rs02MotorDriver(uint16_t motor_id, const std::string& can_interface,
    ...
    can_->add_can_callback(can_callback, RS02_HOST_ID);
}

Rs02MotorDriver::~Rs02MotorDriver() {
    can_->remove_can_callback(RS02_HOST_ID);
}
...
void Rs02MotorDriver::lock_motor() {
    ...
    memset(tx_frame.data, 0xFF, 7);
    tx_frame.data[7] = RS02_CMD_ENABLE;
...
void Rs02MotorDriver::unlock_motor() {
    ...
    memset(tx_frame.data, 0xFF, 7);
    tx_frame.data[7] = RS02_CMD_DISABLE;
...
void Rs02MotorDriver::set_motor_control_mode(uint8_t motor_control_mode) {
    if (motor_control_mode > RS02_MODE_SPD) {
    ...
    // RS02 mode mapping: MIT=0, POS=1, SPD=2 matches MotorControlMode_e
    uint8_t rs02_mode = motor_control_mode;  // same values

    if (motor_control_mode == MIT && old_mode != MIT) {
    ...
    tx_frame.data[6] = rs02_mode;
    tx_frame.data[7] = RS02_CMD_SET_MODE;
...
void Rs02MotorDriver::set_motor_zero_rs02() {
    ...
    memset(tx_frame.data, 0xFF, 7);
    tx_frame.data[7] = RS02_CMD_SET_ZERO;
